...
const edit = ref(null)
onMounted(() => {
  tour.edit = edit.value
})
</script>

<template>
  <div ref="editbox" style="background-color: #fffffe" class="h-full w-full">
    <div ref="edit" class="h-full w-full">
      <!-- 最顶一层 -->
      <!-- 第二层 -->
      <div class="optionchoose h-11 w-full flex justify-between bg-gray-100">
        ...
        <div class="flex">
          ...
          <Optionchoose>
            <template #up>
              <div
                class="i-ic:outline-add-circle-outline ml--5 mt-1.7 inline-block h-5 w-5 text-black"
              ></div>
            ...
            <template #down>
              <div
                class="h-33.5 w-40 overflow-hidden rounded-md bg-light-100 p-2 shadow-lg divide-y divide-gray-100"
              >
                <span
                  v-for="(method, addoption) of addoptions"
                  :key="addoption"
                  class="hover:text-gray-90 block h-10 cursor-pointer rounded-lg pt-2.4 text-sm text-gray-500 hover:bg-gray-100"
                  dark="text-light-500 hover:text-light-900 hover:bg-gray-400"
                  @click="method"
                >
                  <span
                    :class="differenticons(addoption)"
                    class="mb-0.6 ml-1 inline-block h-5 w-5 justify-center align-middle"
                  ></span>
                  {{ addoption }}
                </span>
              </div>
          ...
          <button
            ref="target"
            class="bg-color-#f8f9fa h-11 w-12 cursor-pointer border-0 rounded-md p-2 text-gray-600 hover:bg-gray-200 hover:text-gray-700"
            dark="bg-transparent hover:bg-gray-500"
        ...
        <div
          name="languageoption"
          class="bord float-right h-full w-42 cursor-pointer border-1 rounded-lg bg-light-100 text-size-4 text-color-#343a40"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 32 32"
            class="inline-block h-11 w-8"
          >
            <path
              fill="#659ad2"
