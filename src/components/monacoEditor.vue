<template>
  <div ref="main" class="h-full w-full">
    <div ref="vim" class="bg-gray-200 w-full none" v-if="!status">
      --NORMAL--
    </div>
  </div>
</template>
...
const init = () => {
  ...
  monacoEditor = monaco.editor.create(main.value, {
    theme: 'vs-light', // 主题
    value: real_compliercode.value, // 默认显示的值
    language: 'cpp',
    automaticLayout: true, // 自适应布局
    fontSize: real_fontsize.value, // 字体大小
    ...
    readOnly: props.permit, // 是否为只读模式
  })
  watch(
    () => real_compliercode.value,
    (newVal) => {
  ...
  monacoEditor.onDidChangeModelContent(() => {
    if (!props.permit) {
      // 只有当 isUserInput 为 true 时，才触发下面的代码
...
const toggle = () => {
  if (status.value) {
    vimMode = initVimMode(monacoEditor, vim.value)
  } else {
    vimMode.dispose()
  }
...
watch(
  () => props.fontsize,
  () => {
    real_fontsize.value = props.fontsize
